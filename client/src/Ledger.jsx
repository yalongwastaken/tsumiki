...
export default function Ledger({ transactions, sources, onDelete }) {
  ...
  return <>
    ...
    {rows.length === 0 ? (
      <div className="text-center py-12 text-slate-400 text-sm">Nothing logged yet. Tap <span className="font-semibold text-brand-600">+</span> to start.</div>
    ) : (
        ...
        {rows.map((t) => (
          <div key={t.id} className="flex items-center justify-between px-4 py-2.5">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => onDelete(t.id)} aria-label="Delete" className="text-slate-300 hover:text-rose-400"><X size={15} /></button>
            </div>
          </div>
        ))}
      </div>
