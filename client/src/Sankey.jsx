...
export default function SankeyFlow({ transactions, fallbackIncome }) {
  ...
  const items = [
    ...Object.entries(contribMap).map(([b, a]) => ({
      label: bucketLabel(b),
      amount: a,
      color: bucketColor(b),
    })),
  ...
  if (freeAmt > 0) {
    items.push({ label: "Leftover", amount: freeAmt, color: "#94A3B8" });
  }

  let ry = PTOP;
  const right = items.map((it) => {
    const h = MIN_H + (it.amount / income) * SCALE;
    const r = { ...it, y: ry, h };
    ry += h + GAP;
    return r;
  });
  const SVG_H = ry - GAP + PBOT,
    leftH = SVG_H - PTOP - PBOT;
  let ly = PTOP;
  const left = items.map((it) => {
    const h = (it.amount / income) * leftH;
    const b = { ...it, y: ly, h };
    ly += h;
    return b;
  });
  const ribbon = (l, r) => {
    const cx = (RX - LX - LW) * 0.42,
      x1 = LX + LW,
      x2 = RX;
  ...
  return (
    ...
    >
      ...
      {right.map((r, i) => (
        <path key={i} d={ribbon(left[i], r)} fill={r.color} fillOpacity={0.2} />
      ))}
      {right.map((r, i) => (
        <rect key={i} x={RX} y={r.y} width={RW} height={r.h} fill={r.color} rx={2} />
      ))}
      {right.map((r, i) => {
        const m = r.y + r.h / 2,
          lc = r.color === "#94A3B8" ? "var(--muted)" : r.color;
        return (
          <g key={i}>
            <text
              x={RX + RW + 10}
