...
// A donut of where the plan sends this paycheck (grouped into the four
// destinations + essentials/debt), plus the alternative strategy splits.
...
const STRATS = [
  ["short_term", "Safety", { savings: 0.35, retirement: 0.20, invest: 0.15, checking: 0.30 }],
  ["balanced", "Balanced", { savings: 0.25, retirement: 0.30, invest: 0.30, checking: 0.15 }],
  ["long_term", "Growth", { savings: 0.10, retirement: 0.40, invest: 0.40, checking: 0.10 }],
];
const SPLIT_SEG = [
...
export default function PlanSplitChart({ plan, strategy, saved, onSetStrategy }) {
  ...
  const segs = GROUPS.map((g) => ({ ...g, amount: g.keys.reduce((a, k) => a + (byKey[k] || 0), 0) })).filter((g) => g.amount > 0);
  const total = segs.reduce((a, s) => a + s.amount, 0) || 1;

  const R = 46, SW = 18, C = 2 * Math.PI * R, cx = 60, cy = 60;
  let off = 0;

  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4">
      <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Where it splits</div>
      <div className="flex items-center gap-4 flex-wrap">
        <svg width="120" height="120" viewBox="0 0 120 120" className="flex-shrink-0">
          {segs.map((s, i) => {
            const frac = s.amount / total, dash = frac * C, el = (
              <circle key={i} r={R} cx={cx} cy={cy} fill="none" stroke={s.color} strokeWidth={SW}
                strokeDasharray={`${dash} ${C - dash}`} strokeDashoffset={-off} transform={`rotate(-90 ${cx} ${cy})`} />
            );
            off += dash; return el;
          })}
          <text x={cx} y={cy - 4} textAnchor="middle" fontSize="12" fill="var(--muted)">paycheck</text>
          <text x={cx} y={cy + 12} textAnchor="middle" fontSize="14" fontWeight="600" fill="var(--text)">{fmt(total)}</text>
        </svg>
        <div className="flex-1 min-w-[150px] space-y-1.5">
          {segs.map((s, i) => (
            <div key={i} className="flex items-center gap-2 text-sm">
              ...
              <span className="font-mono text-slate-500">{Math.round((s.amount / total) * 100)}%</span>
      ...
      <div className="mt-4 pt-3 border-t border-slate-50">
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2 text-xs text-slate-400">
          {SPLIT_SEG.map(([k, color]) => <span key={k}><span className="inline-block w-2 h-2 rounded-sm mr-1" style={{ background: color }} />{k}</span>)}
        </div>
      </div>
    </div>
  );
