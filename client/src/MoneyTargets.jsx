...
export default function MoneyTargets({ targets = [], onChange }) {
  ...
  function add() {
    ...
    if (!(amount > 0)) return;
    onChange([...targets, { id: uid(), label: form.label.trim() || `${fmt(amount)} ${metricLabel(form.metric)}`, amount, metric: form.metric }]);
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4">
      <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Your money goals</div>
      {targets.length > 0 && (
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input type="number" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} placeholder="Target amount"
            className="w-full pl-7 pr-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 text-slate-700" />
        </div>
        <button onClick={add} className="px-4 py-2 bg-slate-700 hover:bg-slate-800 text-white text-sm font-semibold rounded-lg">Add</button>
      </div>
    </div>
  );
}
