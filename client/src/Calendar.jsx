...
// Calendar: one month grid combining all four views —
//  • spending heatmap (day cell tint)
//  • activity dots (income = green, contribution = indigo)
//  • bill due-dates (amber dot, from profile.bills[].dayOfMonth)
//  • per-week adherence marker (🔥 if that week's rotating objective was met)
//  • tap a day to see what happened that day
...
export default function Calendar({ transactions = [], profile = {} }) {
  ...
  const byDay = useMemo(() => {
    ...
    for (const t of transactions) {
      ...
      if (x.getFullYear() !== year || x.getMonth() !== month) continue;
      const d = x.getDate();
      (m[d] ??= { spend: 0, income: 0, contrib: 0, items: [] });
      if (t.type === "spending") m[d].spend += t.amount;
      else if (t.type === "income") m[d].income += t.amount;
      else if (t.type === "contribution") m[d].contrib += t.amount;
      m[d].items.push(t);
    }
    return m;
  ...
  const billsByDay = useMemo(() => {
    const m = {};
    for (const b of profile.bills || []) if (b.dayOfMonth) (m[b.dayOfMonth] ??= []).push(b);
    return m;
  }, [profile.bills]);

  // adherence by week (uses all transactions, since a week can span months)
  const weekTx = useMemo(() => {
    const m = {}; for (const t of transactions) (m[weekKey(t.date)] ??= []).push(t); return m;
  }, [transactions]);
  ...
  const tint = (spend) => (spend > 0 ? { background: `rgba(244,63,94,${0.08 + 0.5 * (spend / maxSpend)})` } : undefined);
    ...
    <>
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        <div className="flex items-center justify-between mb-3">
          <button onClick={() => { setOffset(offset - 1); setSel(null); }} className="px-2 py-1 text-slate-400 hover:text-slate-700">‹</button>
        ...
        <div className="grid mb-1" style={{ gridTemplateColumns: "1.2rem repeat(7, 1fr)", gap: 2 }}>
          <div />
          {DOW.map((d, i) => <div key={i} className="text-center text-xs text-slate-300">{d}</div>)}
        </div>

        {rows.map((row, r) => (
          <div key={r} className="grid items-stretch" style={{ gridTemplateColumns: "1.2rem repeat(7, 1fr)", gap: 2, marginBottom: 2 }}>
            <div className="flex items-center justify-center text-xs" title="this week's objective">{weekMet(row.wk) ? "🔥" : <span className="text-slate-200">·</span>}</div>
            {row.days.map((d, c) => {
              if (d == null) return <div key={c} />;
              const info = byDay[d];
      ...
      {sel != null && (
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          {selBills.length > 0 && selBills.map((b) => (
            <div key={b.id} className="flex justify-between py-1.5 text-sm">
              <span className="text-amber-600">Bill due · {b.name}</span><span className="font-mono text-slate-500">{fmt(b.amount)}</span>
            </div>
          ))}
