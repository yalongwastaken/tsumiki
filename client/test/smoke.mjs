// smoke.mjs — headless render smoke test. Mounts <App/> in jsdom with stubbed
// data and walks the tabs, failing if anything throws or a tab renders blank.
// catches the "undefined variable → blank white screen" class of bug that a
// normal build does NOT catch (building ≠ rendering).
//
...
globalThis.ResizeObserver = class {
  ...
  unobserve() {}
  disconnect() {}
}; // jsdom lacks it; recharts needs it
...
const STATE = {
  rev: 1,
  accounts: [
    { id: "chk", name: "Checking", type: "checking", color: "#94A3B8" },
    { id: "sav", name: "Savings", type: "savings", color: "#94A3B8" },
  ],
  snapshots: [
    { id: "s1", accountId: "chk", date: "2026-04-01T00:00:00Z", balance: 3000 },
    { id: "s2", accountId: "sav", date: "2026-06-01T00:00:00Z", balance: 9000 },
  ],
  goals: [
    {
      id: "japan",
...
const PLAN = {
  ...
  leftover: 0,
  investable: 2000,
  steps: [{ key: "brokerage", label: "Invest", amount: 2000, why: "x" }],
};
globalThis.fetch = async (url) => ({
  ok: true,
  status: 200,
  json: async () => (String(url).includes("/api/plan") ? PLAN : STATE),
  text: async () => "",
});
...
const btnByText = (txt, root = document) =>
  [...root.querySelectorAll("button")].find((b) => b.textContent.trim() === txt);

try {
  // open quick-add, switch types, enter an amount, log
  ...
  for (const t of ["Income", "Contribution", "Spending"]) {
    const b = btnByText(t, dialog);
    if (b) {
      await act(async () => b.click());
    }
  }
  ...
  if (amt) {
    await act(async () => setValue(amt, "50"));
  }
  const logBtn = [...dialog.querySelectorAll("button")].find((b) =>
  ...
  if (document.querySelector('[role="dialog"]')) {
    fails.push("quick-add: sheet didn't close after logging");
  } else {
    console.log("  ✓ quick-add: log");
  }
} catch (e) {
  fails.push(`quick-add interaction: ${e.message}`);
}

try {
  // Activity → List view → delete a ledger row
  await navTo("Activity");
  const listBtn = btnByText("List");
  if (listBtn) {
  ...
  if (!del) {
    fails.push("ledger: no delete button found");
  } else {
    await act(async () => del.click());
    await new Promise((r) => setTimeout(r, 80));
    console.log("  ✓ activity: list delete");
  }
} catch (e) {
  fails.push(`activity interaction: ${e.message}`);
}

try {
  // Settings: save profile + dark-mode toggle (now in Appearance)
  await navTo("Settings");
  const save = btnByText("Save profile");
  if (!save) {
    fails.push("settings: no Save profile button found");
  } else {
    await act(async () => save.click());
    await new Promise((r) => setTimeout(r, 80));
    console.log("  ✓ settings: save profile");
  }
  const darkBtn = btnByText("Dark");
  if (!darkBtn) {
    fails.push("theme: no Dark button in Settings");
  } else {
    ...
    if (document.documentElement.classList.contains("dark")) {
      ...
      fails.push("theme: .dark class not applied");
    }
  }
} catch (e) {
  fails.push(`settings interaction: ${e.message}`);
}

if (errors.length) {
  fails.push(...errors.map((e) => `render error: ${e}`));
}
if (fails.length) {
  console.error("\nSMOKE TEST FAILED:\n - " + fails.join("\n - "));
  process.exit(1);
