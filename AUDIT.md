...
Status of every finding, verified against the code and `docs/CHANGELOG.md` (released as v2.5.0):
...
| ❌ Open             | **M8**/**M9** (offline read/write resilience), **M10** remainder (debt/bill/income/target deletes are still one-tap), **L7** (persist login throttle), **L8** (warn on ambiguous European CSV formats)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
Feature opportunities shipped 2026-07-13: #1 undo-delete, #2 backdate in QuickAdd, #5 month-in-review card (plus #9/#11/#12, which landed earlier). Remaining open items are offline-resilience work (M8/M9), confirm-or-undo for the remaining setup editors (M10 remainder), the login-throttle persistence note (L7), and CSV format warnings (L8).
